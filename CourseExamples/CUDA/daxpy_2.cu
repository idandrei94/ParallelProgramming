 ...
 * Subprogram library (BLAS). This version allows an arbitrary
 * number of threads per thread block and computes the necessary
 * number of thread blocks per grid so that at least one thread
 * for each vector element is available.
 *
 *
 * Compiling:
 *   nvcc -arch=sm_50 -o daxpy_2 daxpy_2.cu
 *   clang --cuda-gpu-arch=sm_50 -o daxpy_2 daxpy_2.cu -lcudart
 *
 * Running:
 *   ./daxpy_2
 *
 *
 * File: daxpy_2.cu			Author: S. Gross
 * Date: 14.02.2018
 *
 */
...
int main (void)
{
  ...
  /* Determine "number of thread blocks per grid" so that
   * blocks_per_grid * THREADS_PER_BLOCK >= VECTOR_SIZE
   * i.e., at least one thread for each vector element.
   */
  blocks_per_grid = (VECTOR_SIZE + THREADS_PER_BLOCK - 1) /
		    THREADS_PER_BLOCK;
