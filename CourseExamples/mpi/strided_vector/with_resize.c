 ...
 * "MPI_Type_vector" and will resize the data type so that the
 * program will work as expected. The new data type knows the
 ...
 *
 *
 * File: with_resize.c			Author: S. Gross
 * Date: 04.08.2017
 *
 */
...
int main (int argc, char *argv[])
{
  ...
  if (mytid == 0)
  {
    MPI_Type_get_extent (tmp_column_t, &lb_of_tmp_column_t,
			 &extent_of_tmp_column_t);
    MPI_Type_get_extent (column_t, &lb_of_column_t,
			 &extent_of_column_t);
    printf ("extent of type tmp_column_t:      %d\n"
	    "lower bound of type tmp_column_t: %d\n\n"
	    "extent of type column_t:          %d\n"
	    "lower bound of type column_t:     %d\n\n",
	    (int) extent_of_tmp_column_t, (int) lb_of_tmp_column_t,
	    (int) extent_of_column_t, (int) lb_of_column_t);
  }
  MPI_Barrier (MPI_COMM_WORLD);		/* wait for process with rank 0	*/
