...
int main (int argc, char *argv[])
{
  ...
  if (ntasks > Q)
  {
    ...
    group_w_mem = (int *) malloc ((size_t) num_worker * sizeof (int));
    TestEqualsNULL (group_w_mem);	/* test if memory was available	*/
    if (mytid == 0)
    {
      printf ("\nYou have started %d processes but I need at most "
	      "%d processes.\n", ntasks, Q);
      if (*universe_size_ptr != 1)
      {
	printf ("The universe contains %d \"virtual cpu's\" (\"0\" means "
		"universe size isn't available).\n", *universe_size_ptr);
      }
      else
      {
	printf ("The universe contains %d \"virtual cpu\" (\"0\" means "
		"universe size isn't available).\n", *universe_size_ptr);
      }
      if (num_worker > 1)
      {
	printf ("I build a new worker group with %d processes. The "
		"processes with\n"
      ...
      {
	printf ("I build a new worker group with %d process. The "
		"process with\n"
		"the following rank in the basic group belongs to "
		"the new group:\n  ", num_worker);
      }
    }
  ...
  if (group_w_mytid != MPI_UNDEFINED)
  {
     ...
     */
    num_columns = (int *) malloc ((size_t) group_w_ntasks * sizeof (int));
    ...
    if (group_w_mytid == 0)
    {
       ...
       */
      sr_counts = (int *) malloc ((size_t) group_w_ntasks * sizeof (int));
      TestEqualsNULL (sr_counts);
      sr_disps = (int *) malloc ((size_t) group_w_ntasks * sizeof (int));
     ...
     */
    col_block =
      (double **) malloc ((size_t) (P * num_columns[group_w_mytid]) *
				    sizeof (double));
