 ...
 *   mpijavac ColumnSendRecvInt2DarrayIn1DarrayMain.java
 *
 * Usage:
 *   mpiexec [parameters] java [parameters] \
 *	ColumnSendRecvInt2DarrayIn1DarrayMain
 *
 * Examples:
 *   mpiexec -np 7 java ColumnSendRecvInt2DarrayIn1DarrayMain
 *   mpiexec -np 7 java -cp $HOME/mpi_classfiles \
 *	ColumnSendRecvInt2DarrayIn1DarrayMain
 *
 *
 * File: ColumnSendRecvInt2DarrayIn1DarrayMain.java	Author: S. Gross
 * Date: 10.09.2013
 *
 */
...
public class ColumnSendRecvInt2DarrayIn1DarrayMain
{
						 ...
						 InterruptedException
  {
     ...
     * process "i" so that we must use an IntBuffer instead of an
     * array for the send-operation.
     */
    IntBuffer matrixBuffer;
    ...
    MPI.Init (args);
    matrixBuffer = MPI.newIntBuffer(P * Q);
    ...
    if (ntasks != (Q + 1))
    {
      ...
      {
	String className =
	  new ColumnSendRecvInt2DarrayIn1DarrayMain().getClass().getName();
	System.err.printf ("\nI need %d processes.\n" +
			   "Usage:\n" +
			   "  mpiexec -np %d java %s\n\n",
			   Q + 1, Q +  1, className);
    ...
    tmp_column_t = Datatype.createVector (P, 1, Q, MPI.INT);
    column_t     = Datatype.createResized (tmp_column_t, 0, SIZEOF_INT);
    column_t.commit ();
    tmp_column_t.free ();
    if (mytid == 0)
    {
    ...
    {
      MPI.COMM_WORLD.recv (column, P, MPI.INT, 0, 0);
