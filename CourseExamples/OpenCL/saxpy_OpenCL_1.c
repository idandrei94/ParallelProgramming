 ...
 * Compiling:
 *   Linux:
 *     gcc -o saxpy_OpenCL_1 saxpy_OpenCL_1.c errorCodes.c -lOpenCL
 *   Mac OS X:
 *	 Uses frameworks:
 *	   -framework OpenCL
 *	   -framework OpenGL -framework GLUT
 *     gcc -o saxpy_OpenCL_1 saxpy_OpenCL_1.c errorCodes.c \
 *	   -framework OpenCL
 *   Windows:
 *     cl saxpy_OpenCL_1.c errorCodes.c OpenCL.lib
 *
 * Running:
 *   ./saxpy_OpenCL_1
 *
 *
 * File: saxpy_OpenCL_1.c		Author: S. Gross
 * Date: 12.06.2017
 *
 */
 ...
 *
 * void saxpy (int n, float alpha, float x[], int incx,
 *	       float y[], int incy);
 *
 *
...
int main (void)
{
  ...
  for (unsigned int platform = 0; platform < numPlatforms; ++platform)
  {
	...
	(const char **) &kernelSource, NULL, &errcode_ret);
      CheckRetValueOfOpenCLFunction (errcode_ret);
