 ...
 * Compiling:
 *   Linux:
 *     gcc -o index index.c errorCodes.c -lOpenCL
 *   Mac OS X:
 *	 Uses frameworks:
 *	   -framework OpenCL
 *	   -framework OpenGL -framework GLUT
 *     gcc -o index index.c errorCodes.c -framework OpenCL
 *   Windows:
 *     cl index.c errorCodes.c OpenCL.lib
 *
 * Running:
 *   ./index
 *
 *
 * File: index.c			Author: S. Gross
 * Date: 12.06.2017
 *
 */
...
int main (void)
{
  ...
  for (unsigned int platform = 0; platform < numPlatforms; ++platform)
  {
	...
	command_queue = clCreateCommandQueueWithProperties (context,
      ...
      {
	/* "CL_VERSION_2_0" reported, but the device supports only
	 * OpenCL 1.2
	 */
	if (outOfOrderQueue == 1)
	{
