 ...
 *
 * This version uses OpenMP and the "critical" directive.
 *
 ...
 *
 * Compiling:
 *   cc  -xopenmp -o pi_critical pi_critical.c [omp_stubs.c] [-lrt]
 *   gcc -fopenmp -o pi_critical pi_critical.c [omp_stubs.c] [-lrt]
 *   icc -oqpenmp -o pi_critical pi_critical.c [omp_stubs.c]
 *   cl  /GL /Ox /openmp pi_critical.c [omp_stubs.c]
 *
 * Running:
 *   ./pi_critical [number of intervals]
 *
 *
 * File: pi_critical.c			Author: S. Gross
 * Date: 04.08.2017
 *
 */
...
int main (int argc, char *argv[])
{
  ...
  #pragma omp parallel default(none) private(i, x, fx)	\
    shared(h, h2, num_iter, num_threads, pi)
  {
    #pragma omp single
    {
