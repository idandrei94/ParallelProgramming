 ...
 *
 * This version uses OpenMP and "lock" functions from the OpenMP
 * library.
 *
 ...
 *
 * Compiling:
 *   cc  -xopenmp -o pi_lock_optimized pi_lock_optimized.c \
 *	 [omp_stubs.c] [-lrt]
 *
 *   gcc -fopenmp -o pi_lock_optimized pi_lock_optimized.c \
 *	 [omp_stubs.c] [-lrt]
 *
 *   icc -qopenmp -o pi_lock_optimized pi_lock_optimized.c \
 *	 [omp_stubs.c]
 *
 *   cl  /GL /Ox /openmp pi_lock_optimized.c [omp_stubs.c]
 *
 * Running:
 *   ./pi_lock_optimized [number of intervals]
 *
 *
 * File: pi_lock_optimized.c		Author: S. Gross
 * Date: 04.08.2017
 *
 */
...
int main (int argc, char *argv[])
{
  ...
  omp_lock_t add_to_pi_lock;		/* avoid race conditions	*/
  ...
  cpu_time   = clock ();
  omp_init_lock (&add_to_pi_lock);
  pi = 0.0;
  fx = 0.0;
  h  = 1.0 / (double) num_iter;
  h2 = h / 2;
  #pragma omp parallel default(none) private(i, x) firstprivate(fx) \
    shared(add_to_pi_lock, h, h2, num_iter, num_threads, pi)
  {
    #pragma omp single
    {
    ...
    omp_set_lock (&add_to_pi_lock);
    pi += fx;
    omp_unset_lock (&add_to_pi_lock);
  }
  omp_destroy_lock (&add_to_pi_lock);
  end_wall = time (NULL);
