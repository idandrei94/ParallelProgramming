 ...
 *
 * This version uses OpenMP with a "parallel" directive, but
 * without a "for" directive, so that every thread computes
 * everything. The result will be wrong, because all threads
 * manipulate vector "y" without synchronization.
 *
 ...
 *
 * Compiling:
 *   cc  -xopenmp -o daxpy_wrong_parallel daxpy_wrong_parallel.c \
 *	 [omp_stubs.c] [-lrt]
 *
 *   gcc -fopenmp -o daxpy_wrong_parallel daxpy_wrong_parallel.c \
 *	 [omp_stubs.c] [-lrt]
 *
 *   icc -qopenmp -o daxpy_wrong_parallel daxpy_wrong_parallel.c \
 *	 [omp_stubs.c]
 *
 *   cl  /GL /Ox /openmp daxpy_wrong_parallel.c [omp_stubs.c]
 *
 * Running:
 *   ./daxpy_wrong_parallel [size of vector]
 *
 *
 * File: daxpy_wrong_parallel.c		Author: S. Gross
 * Date: 04.08.2017
 *
 */
