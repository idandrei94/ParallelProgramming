 ...
 * vector. This version initializes the vector with a more
 * complicated value. Vectors with more than 100.000 elements
 * may profit from a parallel initialization.
 *
 ...
 * Compiling:
 *
 * cc  -xopenmp -o omp_parallel_for_2 omp_parallel_for_2.c
 * gcc -fopenmp -o omp_parallel_for_2 omp_parallel_for_2.c
 * icc -qopenmp -o omp_parallel_for_2 omp_parallel_for_2.c
 * cl  /GL /Ox /openmp omp_parallel_for_2.c
 *
 *
 * Running:
 *   ./omp_parallel_for_2 [size of vector]
 *
 *
 * File: omp_parallel_for_2.c		Author: S. Gross
 * Date: 04.08.2017
 *
 */
...
#define DEF_VECTOR_SIZE 100000		/* default vector size		*/
#define MAX_RANDOM_NUM  1000		/* maximum random number	*/
...
int main (int argc, char *argv[])
{
  ...
  for (i = 0; i < size; ++i)
  {
    a[i] = (double) ((rand () % MAX_RANDOM_NUM) *
		     (rand () % MAX_RANDOM_NUM) *
		     (rand () % MAX_RANDOM_NUM));
    a[i] *= (double) ((rand () % MAX_RANDOM_NUM) *
		      (rand () % MAX_RANDOM_NUM) *
