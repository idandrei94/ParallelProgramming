 ...
 *
 * Compiling:
 *   mpicc [-DP=<value>] [-DQ=<value>] [-DR=<value>] \
 ...
 *
 *
 * File: mat_mult_int_mpi_ijk.c		Author: S. Gross
 * Date: 16.07.2018
 *
 */
...
int main (int argc, char *argv[])
{
  ...
  double  t_comm, t_comm_tmp,		/* time for communication	*/
	  ct_mult_ab, my_ct_mult;	/* cpu time for multiplication	*/
  time_t  st_mult_ab, et_mult_ab;	/* start/end time (wall clock)	*/
  clock_t my_ct_mult_ab;		/* used cpu time		*/
  ...
  MPI_Bcast (b, Q * R, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  row_a = (int **) malloc ((size_t) (num_rows[mytid] * Q) * sizeof (int)); 
  TestEqualsNULL (row_a);
  row_c = (int **) malloc ((size_t) (num_rows[mytid] * R) * sizeof (int)); 
  TestEqualsNULL (row_c);
  /* send row block i of "a" to process i				*/
  MPI_Scatterv (a, sr_counts, sr_disps, MPI_INT, row_a,
		num_rows[mytid] * Q, MPI_INT, 0, MPI_COMM_WORLD);
   ...
   * "(int *) mat + i * q + j" and mat[i][j] itself as
   * "*((int *) mat + i * q + j)".
   */
  my_ct_mult_ab = clock ();
  for (i = 0; i < num_rows[mytid]; ++i)
  {
    for (j = 0; j < R; ++j)
    {
      *((int *) row_c + i * R + j) = 0;		/* row_c[i][j] = 0	*/
      for (k = 0; k < Q; ++k)
      {
	/* row_c[i][j] += row_a[i][k] * b[k][j]				*/

	*((int *) row_c + i * R + j) +=
	  *((int *) row_a + i * Q + k) * b[k][j];
      }
    }
  }
  my_ct_mult_ab = clock () - my_ct_mult_ab;
  /* different machines may have different clock resolutions		*/
  my_ct_mult	= (double) my_ct_mult_ab / CLOCKS_PER_SEC;
  ...
  MPI_Gatherv (row_c, num_rows[mytid] * R, MPI_INT, c, sr_counts,
	       sr_disps, MPI_INT, 0, MPI_COMM_WORLD);
  t_comm += (MPI_Wtime () - t_comm_tmp);
  et_mult_ab = time (NULL);
  /* collect the computation time for the matrix multiplication
   * from all processes
   */
  MPI_Reduce (&my_ct_mult, &ct_mult_ab, 1, MPI_DOUBLE, MPI_SUM,
	      0, MPI_COMM_WORLD);
