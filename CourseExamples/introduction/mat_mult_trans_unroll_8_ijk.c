...
int main (void)
{
  ...
  /* Q should be a multiple of 8 to keep unrolling simple		*/
  if ((Q % 8) != 0)
  {
    fprintf (stderr, "\"Q = %d\" is not a multiple of 8. Please "
	     "choose for Q\n"
  ...
  for (i = 0; i < P; ++i)		/* compute matrix c		*/
  {
	...
	c[i][j] += ((a[i][k] * bt[j][k]) + (a[i][k+1] * bt[j][k+1]) +
		    ...
		    (a[i][k+4] * bt[j][k+4]) + (a[i][k+5] * bt[j][k+5]) +
		    (a[i][k+6] * bt[j][k+6]) + (a[i][k+7] * bt[j][k+7]));
      }
    }
  }
