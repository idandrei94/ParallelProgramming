 ...
 * The algorithm uses submatrix-multiplications so that it can profit
 * from cache hits reducing the overall execution time (if all three
 * current submatrices fit into the cache). Furthermore it uses the
 * "traditional" way multiplying and adding the elements of one row
 * and one column within each submatrix to determine one element of
 * a submatrix of the result matrix "c".
 * It uses virtual addresses so that its results may differ from
 * real computations where the virtual addresses of the matrix
 ...
 *		  -DBLOCKSIZE=48 -DSET_HITS_MISSES -DFIRST_COLUMN \
 *		  -o mat_mult_block_cacheline_ijk \
 *		  mat_mult_block_cacheline_ijk.c -lm
 *
 * Running:
 *   <program name>
 *
 *
 * File: mat_mult_block_cacheline_ijk.c	Author: S. Gross
 * Date: 07.08.2017
 *
 */
...
#endif
#ifndef BLOCKSIZE
  #define BLOCKSIZE	48		/* # columns/rows in submatrix	*/
#endif
...
int main (void)
{
   ...
   * split rows in BLOCKSIZE blocks
   */
  for (ib = 0; ib < P; ib += BLOCKSIZE)
  {
    /* split columns in BLOCKSIZE blocks				*/
    #ifndef FIRST_COLUMN
      for (jb = 0; jb < R; jb += BLOCKSIZE)
    #else
      for (jb = 0; jb < 1; jb += BLOCKSIZE)
    #endif
    {
      /* split columns/rows in BLOCKSIZE blocks				*/
