 ...
 * into the cache). Furthermore it uses the "traditional" way of
 ...
 * until the final values of c are determined. In this version the
 * k-loop is peeled into a major loop where the number of cycles is
 * always a multiple of the STRIDESIZE and possibly a small loop for
 * the remaining elements. so that it isn't necessary to determine the
 * upper bound for the last iteration as minimum of two values.
 *
 ...
 *
 *
 * File: mat_mult_stride_peel_ikj.c	Author: S. Gross
 * Date: 04.08.2017
 *
 */
...
int main (void)
{
  int	  i, j, k, kb,			/* loop variables		*/
	  k_upper_bound,		/* upper bound for k		*/
	  ok;				/* temporary value		*/
  ...
  k_upper_bound = (Q / STRIDESIZE) * STRIDESIZE;
  for (i = 0; i < P; ++i)
  {
    /* split column/row in STRIDESIZE blocks and use loop peeling
     * to avoid upper bound computations
     */
    for (kb = 0; kb < k_upper_bound; kb += STRIDESIZE)
    {
      for (k = kb; k < (kb + STRIDESIZE); ++k)
      {
	for (j = 0; j < R; ++j)
	{
	  c[i][j] += a[i][k] * b[k][j];
    ...
    /* perhaps we have some additional rows if STRIDESIZE isn't
     * a factor of Q
     */
    for (k = k_upper_bound; k < Q; ++k)
    {
      for (j = 0; j < R; ++j)
      {
	c[i][j] += a[i][k] * b[k][j];
